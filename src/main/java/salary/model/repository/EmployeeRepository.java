...
public class EmployeeRepository implements Repository<Employee> {
    ...
    public void save(Employee employee) throws Exception {
        employee.setId(ConnectionProvider.getConnectionProvider().getNextId(connection, "employee_seq"));
        ...
        preparedStatement.setString(6, employee.isMarried() ? "Y" : "N");
    ...
    public void edit(Employee employee) throws Exception {
        preparedStatement = connection.prepareStatement(
                "update employees set first_Name=?,last_Name=?,national_Id=?,education=?,married=?,number_Of_Children=?,gender=?,birth_Date=?,insurance_Number=?,bank_Account_Number=?,job_Title=?,position=?,hire_Date=?,termination_Date=?,daily_Salary=? where id=?"
        );
        ...
        preparedStatement.setString(5, employee.isMarried() ? "Y" : "N");
        ...
        preparedStatement.setString(11, employee.getJobTitle().name());
        preparedStatement.setString(12, employee.getPosition().name());
        preparedStatement.setDate(13, (employee.getHireDate() != null) ? Date.valueOf(employee.getBirthDate()) : null);
        preparedStatement.setDate(14, (employee.getTerminationDate() != null) ? Date.valueOf(employee.getBirthDate()) : null);
        preparedStatement.setDouble(15, employee.getDailySalary());
